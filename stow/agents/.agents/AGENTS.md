...
- Creds in `data/transformation/.env` (SQLMESH*STATE_DB*\*)
...
- Worktrees: create under `.claude/worktrees/<short-name>` inside the repo (not sibling dirs), copy `.claude/settings.local.json` and `.env`
...
- Vault path: `~/obsidian/riethmayer` (same on all machines)
