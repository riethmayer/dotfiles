...
⚠️ **HTML-escape `<`, `>`, `&` inside every `<pre class="mermaid">` block.** The browser parses `<pre>` content as HTML before Mermaid reads `.textContent`, so raw `<<port>>` / `<<usecase>>` / `<name>` / `<br/>` get stripped or rewritten and Mermaid throws "Syntax error in text" on otherwise-valid source. `mmdc` reads files directly so it doesn't hit this — a diagram that validates locally can still fail in the browser. See the "Critical: HTML-escape" section in `mermaid-earlybird-theme.md` for the rule, table, and one-line escape helper. **Always verify with `agent-browser eval` after generating.**
