...
name: turborepo
description: |
  Use when working on Turborepo workspace structure, pipelines, caching, filtering,
  CI, or cross-package dependency wiring.
metadata:
...
1. Create or update a shared package under `packages/*`
2. Reuse logic across multiple apps (especially cross-app auth/data/platform code)
3. Add or fix Turborepo tasks, dependencies, filters, cache behavior, or CI usage
4. Diagnose monorepo issues caused by task ordering, hash inputs, or workspace dependency wiring
5. Spin up a new app and standardize startup/auth environment variables across apps

If the task is a single-app code change with no monorepo/workspace impact, this skill is optional.
...
## Repo Convention: New App Bootstrap (eagleeye monorepo)

When creating a new app in `apps/*`, follow this checklist:

1. Make app ports env-driven in scripts (`PORT` with an app-specific default), not hardcoded numbers.
2. For NextAuth apps, derive `NEXTAUTH_URL` from `PORT` by default.
3. Include `PORT`, `NEXTAUTH_URL`, `AUTH_PROVIDER`, `GOOGLE_CLIENT_ID`, and `GOOGLE_CLIENT_SECRET` in
   the app env template.
4. Reuse the same local Google OAuth client across apps by copying
   `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` from an existing local app env file into the new app's
   `.env.local`.
5. Never commit actual secrets; keep `.env.local` ignored and commit placeholders in `.env.example`.

Use this script template for Next.js apps:

```json
{
  "scripts": {
    "dev": "NEXTAUTH_URL=${NEXTAUTH_URL:-http://localhost:${PORT:-3555}} next dev --turbo --port ${PORT:-3555}",
    "start": "NEXTAUTH_URL=${NEXTAUTH_URL:-http://localhost:${PORT:-3555}} next start --port ${PORT:-3555}"
  }
}
```

`dev`/`start` above use shell parameter expansion (`${...:-...}`), so they require a Unix-like shell
(macOS/Linux/WSL/Git Bash). For native Windows shells, set `PORT` and `NEXTAUTH_URL` in env files
and use scripts without inline expansion.
...
If `dev`/`start` derive `NEXTAUTH_URL` from `PORT`, setting `PORT` alone is sufficient. Keep
`NEXTAUTH_URL` in env files only when an explicit override is needed.
...
(`turbo.json` in each package) instead of cluttering root `turbo.json` with `package#task`
overrides.
