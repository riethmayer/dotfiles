...
Cross-tool AI agent config. `~/.agents/AGENTS.md` is the source of truth for instructions; Claude Code reads it via symlink. Personal skills are checked out separately at `~/skills`.
...
- Agent skill content lives in `~/skills`; agents install it via their own marketplaces (no symlink)
- `mise run skills` clones/updates `~/skills`, registers the Git-backed Codex `jan-skills` marketplace, and installs the background updater
