...
**Date:** 2026-06-07
**Status:** Accepted (amended 2026-07-17, 2026-08-01)
...
`git@github.com:riethmayer/skills.git`, checked out at `~/skills`.

## Decision

Dotfiles owns the installation and exposure points, not the skill content:

- `~/skills` is cloned and fast-forwarded by `personal-skills-sync`.
...
checkout updated. Dirty `~/skills` worktrees are fetched but never
...
- Editing skills happens in `~/skills`.
...
## Amendment (2026-08-01): checkout moved to `~/skills`

The checkout lives at `~/skills` on every machine (was `~/src/my-skills`).
